...
class Source : public Expression
{
  void eval(Array<double>& values, const Array<double>& x) const
  {
    double dx = x[0] - 0.5;
    double dy = x[1] - 0.2;
    values[0] = 20000 * exp(-(dx*dx + dy*dy) * 1000);
    values[1] = 20000 * exp(-(dx*dx + dy*dy) * 1000);
    //values[0] = -pow((dx*dx + dy*dy), -1);
    //values[0] = 1;
    //values[1] = 1;
  }
};
...
class DirichletBoundary : public SubDomain
{
  ...
  {
    return (x[0] < 0.1 + DOLFIN_EPS && 0.4 + DOLFIN_EPS > x[1] && x[1] > 0.6 + DOLFIN_EPS)
        or ((x[1] - 0.1) * (x[1] - 0.1) + (x[0] - 0.5) * (x[0] - 0.5) < 0.05 * 0.05 + DOLFIN_EPS && x[1] > 0.1 + DOLFIN_EPS)
        or x[0] > 1.0 - DOLFIN_EPS or x[1] < DOLFIN_EPS or x[1] > 1.0 - DOLFIN_EPS;
  }
...
int main()
{
  ...
  auto mesh = std::make_shared<Mesh>(
    UnitSquareMesh::create({{64, 64}}, CellType::Type::triangle));
